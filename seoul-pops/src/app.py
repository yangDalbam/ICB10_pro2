...
unique_dongs = sorted(df['행정동코드'].unique())
...
with st.sidebar.form("filter_form"):
    selected_dongs = st.multiselect("🏢 행정동코드 선택", options=unique_dongs, default=[])
...
if selected_dongs:
    filtered_df = filtered_df[filtered_df['행정동코드'].isin(selected_dongs)]
if selected_genders:
...
# Dashboard Main Tabs
# ---------------------------------------------------------
tab1, tab2, tab3 = st.tabs(["📝 데이터 개요 및 통계", "📊 Plotly 다차원 동적 차트 (15+)", "📄 분석 리포트 확인"])
...
with tab2:
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc1}")
    charts_info.append({'title': '성별 데이터 빈도 분석', 'filename': 'chart1_gender_freq.png', 'description': desc1, 'table_md': filtered_df['성별'].value_counts().to_frame().to_markdown()})
    st.markdown("---")
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc2}")
    charts_info.append({'title': '연령대별 데이터 빈도 분석', 'filename': 'chart2_age_freq.png', 'description': desc2, 'table_md': age_counts.set_index('연령대').to_markdown()})
    st.markdown("---")
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc3}")
    charts_info.append({'title': '시간대별 관측 빈도', 'filename': 'chart3_time_freq.png', 'description': desc3, 'table_md': time_counts.set_index('시간대').T.to_markdown()})
    st.markdown("---")
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc4}")
    charts_info.append({'title': '성별 평균 생활인구수', 'filename': 'chart4_gender_pop.png', 'description': desc4, 'table_md': gender_mean.set_index('성별').to_markdown()})
    st.markdown("---")
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc5}")
    charts_info.append({'title': '연령대별 평균 생활인구', 'filename': 'chart5_age_pop.png', 'description': desc5, 'table_md': age_mean.set_index('연령대').to_markdown()})
    st.markdown("---")
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc6}")
    charts_info.append({'title': '시간대별 평균 생활인구수 추이', 'filename': 'chart6_time_pop.png', 'description': desc6, 'table_md': time_mean.set_index('시간대').T.to_markdown()})
    st.markdown("---")
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc7}")
    charts_info.append({'title': '성별 및 시간대별 인구 추이', 'filename': 'chart7_time_gender.png', 'description': desc7, 'table_md': time_gender_pivot.set_index('시간대구분').to_markdown()})
    st.markdown("---")
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc8}")
    charts_info.append({'title': '상위 10개 행정동', 'filename': 'chart8_top10_dong.png', 'description': desc8, 'table_md': dong_mean.set_index('행정동코드').to_markdown()})
    st.markdown("---")
    ...
    with col_table:
        ...
        st.write(f"**해석**: {desc9}")
    charts_info.append({'title': '최상위 5개 행정동 시간대별 추이', 'filename': 'chart9_time_top5dong.png', 'description': desc9, 'table_md': time_dong_pivot.set_index('시간대구분').head().to_markdown()})
    st.markdown("---")
    ...
    charts_info.append({'title': '성별 및 연령대 교차 분석', 'filename': 'chart10_gender_age.png', 'description': desc10, 'table_md': gender_age_pivot.set_index('연령대').to_markdown()})
    st.markdown("---")
    ...
    charts_info.append({'title': '주야간 생활인구 비교', 'filename': 'chart11_ampm.png', 'description': desc11, 'table_md': ampm_mean.set_index('주야간').to_markdown()})
    st.markdown("---")
    ...
    if not sample_df.empty:
        charts_info.append({'title': '연령대별 분포 범위', 'filename': 'chart12_age_boxplot.png', 'description': desc12, 'table_md': sample_df.groupby('연령대')[pop_col].describe().to_markdown()})
    st.markdown("---")
    ...
    charts_info.append({'title': '전체 인구 분포 빈도도', 'filename': 'chart13_pop_hist.png', 'description': desc13, 'table_md': ""})
    st.markdown("---")
    ...
    charts_info.append({'title': '시간대 x 연령대 매트릭스', 'filename': 'chart14_time_age_heatmap.png', 'description': desc14, 'table_md': time_age_pivot.head().to_markdown()})
    st.markdown("---")
    ...
    charts_info.append({'title': '성별 x 주야간 스택 비교', 'filename': 'chart15_gender_ampm_stack.png', 'description': desc15, 'table_md': gender_ampm_pivot.set_index('주야간').to_markdown()})
    st.markdown("---")
