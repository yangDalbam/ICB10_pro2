...
def show_data_source_dialog():
    ...
    본 대시보드는 최근 1년(2025.06~2026.05) 기준의 한국관광 데이터랩(KTO), 공공데이터포털(ODCloud), 신한카드/BC카드 빅데이터, 구글 트렌드 검색어 데이터 및 **문화공공데이터광장(문화체육관광부 추천 여행지 100선)** 데이터를 수집하여 개발되었습니다. 모든 데이터는 정교한 전처리 과정을 거쳐 지역별 및 지표별 통계로 가공되었습니다.
    ...
    - 🏞️ **추천 여행지 및 테마 분석:** 출처: 문화공공데이터광장. 문화체육관광부에서 선정한 추천 여행지 100선 데이터를 활용하여 대도시 외의 숨겨진 로컬 명소 추천 빈도와 핵심 테마(TF-IDF)를 도출했습니다.
    """)
...
def render_home():
    ...
    with col_title:
        ...
        with c_text:
            st.markdown("<p style='font-size: 0.9rem; color: #94A3B8; margin-top: 5px;'>📊 <b>분석 근거 기준:</b> 한국관광공사(데이터랩), 공공데이터포털, 문화공공데이터광장, 구글 트렌드 (모두 최근 1년: 2025.06~2026.05 기준)</p>", unsafe_allow_html=True)
        with c_btn:
