...
def show_data_source_dialog():
    ...
    - 🏨 **OTA 기반 지역별 관광 인프라 현황:** 출처: 글로벌 OTA 플랫폼(Klook, KKday, GetYourGuide). 3대 글로벌 OTA 플랫폼의 방대한 여행 상품 및 리뷰 데이터를 통합 분석하여, 대한민국 주요 거점 지역의 관광 인프라 규모와 실제 외국인 관광객들의 수요(리뷰) 데이터를 다각도로 도출했습니다.
    """)
...
def render_home():
    ...
    with col_title:
        ...
        with c_text:
            st.markdown("<p style='font-size: 0.9rem; color: #94A3B8; margin-top: 5px;'>📊 <b>분석 근거 기준:</b> 한국관광공사(데이터랩), 공공데이터포털, 글로벌 OTA 통합 데이터 (모두 최근 1년: 2025.06~2026.05 기준)</p>", unsafe_allow_html=True)
        with c_btn:
