"""
이 모듈은 정제된 리뷰 데이터를 바탕으로 NMF 토픽 모델링을 수행합니다.
주요 기능:
- 4개의 토픽 추출 및 시각화
...
plt.tight_layout()
topic_img = 'plot_topic_modeling.png'
plt.savefig(os.path.join(IMG_DIR, topic_img), bbox_inches='tight')
plt.close()

write_md(f'![토픽 모델링](../images/{topic_img})\n')

# 토픽별 상위 30개 키워드 표 생성
write_md('### 토픽별 상위 30개 키워드 및 가중치\n')
...
write_md('\n### 토픽별 인사이트 및 주제 정리\n')
write_md('<!-- INSIGHT_PLACEHOLDER -->\n')

# 데이터 상하위 5개 행 가중치 표 생성
write_md('### 데이터 상/하위 5개 행의 토픽 가중치\n')
...
write_md("| 데이터 | 제목 | Topic 1 | Topic 2 | Topic 3 | Topic 4 |")
write_md("|---|---|---|---|---|---|")

for idx, row in df.head(5).iterrows():
    title = str(row['title']).replace('|', '&#124;')
    title = (title[:30] + '...') if len(title) > 30 else title
    t1, t2, t3, t4 = [get_color(row[f'Topic_{i}_Weight']) for i in range(1, 5)]
    write_md(f"| 상위 {idx+1} | {title} | {t1} | {t2} | {t3} | {t4} |")

for i, (idx, row) in enumerate(df.tail(5).iterrows()):
    title = str(row['title']).replace('|', '&#124;')
    title = (title[:30] + '...') if len(title) > 30 else title
    t1, t2, t3, t4 = [get_color(row[f'Topic_{i}_Weight']) for i in range(1, 5)]
    write_md(f"| 하위 {5-i} | {title} | {t1} | {t2} | {t3} | {t4} |")
