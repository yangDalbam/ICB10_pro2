"""
이 모듈은 네이버 뉴스 검색 API를 활용하여 뉴스 검색 결과를 수집하고 보여줍니다.
주요 기능:
- 다중 검색어 입력 및 조회
- 검색어별 뉴스 기사 제공
- HTML 태그 제거 및 날짜 파싱
...
st.set_page_config(page_title="뉴스 검색 분석", page_icon="📰")
st.title("뉴스 검색 분석 📰")
...
with st.form("news_search_form"):
    query_input = st.text_input("검색어 (쉼표로 구분하여 여러 개 입력 가능)", value="경제")
    ...
    submit_button = st.form_submit_button("뉴스 검색")
...
if submit_button:
    ...
    if not queries:
        ...
        all_results = []
        with st.spinner("뉴스 데이터를 검색 중입니다..."):
            ...
            if all_results:
                ...
                df_display["date"] = pd.to_datetime(df_display["pubDate"]).dt.date
                trend_df = df_display.groupby(["date", "query"]).size().reset_index(name="count")
                fig = px.line(trend_df, x="date", y="count", color="query", markers=True, title="날짜별 뉴스 기사 수")
