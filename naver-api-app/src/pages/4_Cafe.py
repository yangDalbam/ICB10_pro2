"""
이 모듈은 네이버 카페글 검색 API를 활용하여 카페글 검색 결과를 수집하고 보여줍니다.
주요 기능:
- 다중 검색어 입력 및 조회
- 검색어별 카페글 게시글 결과 제공
- HTML 태그 제거 및 데이터 시각화
"""
...
st.set_page_config(page_title="카페글 검색 분석", page_icon="☕")
st.title("카페글 검색 분석 ☕")
...
with st.form("cafe_search_form"):
    query_input = st.text_input("검색어 (쉼표로 구분하여 여러 개 입력 가능)", value="부동산")
    ...
    submit_button = st.form_submit_button("카페글 검색")
...
if submit_button:
    ...
    if not queries:
        ...
        all_results = []
        with st.spinner("카페글 데이터를 검색 중입니다..."):
            ...
            if all_results:
                ...
                st.subheader("카페별 검색어 언급량 비교")
                import plotly.express as px
                cafe_trend = df_display.groupby(["cafename", "query"]).size().reset_index(name="count")
                cafe_trend = cafe_trend.sort_values(by="count", ascending=False)
                fig = px.bar(cafe_trend, x="cafename", y="count", color="query", 
                             title="출처 카페별 검색어 언급 수", barmode="stack")
